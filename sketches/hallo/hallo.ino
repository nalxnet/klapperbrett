...
const boolean MICRO[7] = {false, true, true, false, false, true, true};
...
// TODO Optimierung
// * Das mittlere Segment flipt an manchen Stellen nur unzuverlässig zurück. Scheint nicht an der Pulslänge oder dem Abstand des Pulses zu liegen.
...
// TODO Features
// * Funktion, die beliebigen String von oben links nach unten rechts schreibt
// * Funktion, die beliebig langen String in einer Zeile schreibt. Wenn Strng länger als Zeile muss dieser durchlaufen.
...
void setup() {
  ...
  delay(2000); // Zeit zum Spannungsaufbau lassen...
...
void loop() {
  ...
  byte links = tickCounter / 10;
  byte rechts = tickCounter % 10;
  writeNumber(15, links);
  writeNumber(14, rechts);
  tickCounter++;
...
void writeMicro(byte spalte) {
  for (int i = 0; i < 7; i++) {
    writeAt(spalte, i+1, MICRO[i]);
  }
}
